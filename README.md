...
```bash
npm run dist                 # dist/KeepSticky-Setup-yyyy.MM.dd.HH.mm.exe
...
gh release create "v$STAMP" "dist/KeepSticky-Setup-$STAMP.exe" --title "ver. $STAMP"
```

앱은 `app/update-check.js` 의 규칙으로 이 릴리즈를 찾는다: 태그는
`v` + `yyyy.MM.dd.HH.mm` 이어야 하고, 첨부 파일 이름은 `KeepSticky-Setup-*.exe` 여야
...
거는 것은 설치본의 `process.execPath`, 즉 설치 자리
(`%LOCALAPPDATA%\Programs\keep-sticky`)의 exe 다. 자리도 이름도 빌드마다 같아
한 번 걸면 계속 유효하다.

**그래도 뜰 때마다 다시 건다.** 포터블로 쓰던 사용자의 레지스트리에는 아직
`D:\KeepSticky-<스탬프>.exe` 가 걸려 있다. 그 파일은 지워지지 않으므로 실패조차
하지 않고, 재부팅 때마다 조용히 옛 포터블이 뜬다 — 아래 "왜 포터블을 버렸나"의
문제로 그대로 돌아간다. 다시 거는 것이 곧 고치는 것이고, 레지스트리 값 이름은
앱 이름이라 항목이 쌓이지도 않는다.
...
- **왜 포터블을 버렸나** (2026-08-11). electron-builder 의 `portable` 타겟은 실행할
  때마다 자기 자신을 `%TEMP%` 에 풀고, 앱이 끝나면 그 폴더를 지운다
  (`app-builder-lib/templates/nsis/portable.nsi` 의 `RMDir /r $INSTDIR`). 트레이에
  며칠씩 상주하는 이 앱에게는 그 삭제가 **실행 중에** 닥칠 수 있고, 실제로 닥쳤다.
  잠기지 않은 파일만 지워지므로 앱은 계속 돌지만 `resources.pak` 이 사라지는데,
  거기에 크로미엄의 UA 스타일시트(`head, title, style { display:none }`)가 있다.
  그러면 **새로 뜨는 창마다** `<head>` 가 글자로 노출된다 — 이미 떠 있던 창은
  멀쩡하고(스타일시트를 이미 메모리에 갖고 있다), 앱의 개발자 도구 창까지 같이
  깨진다. 진단의 결정적 단서가 그 개발자 도구 창이었다: 우리 코드가 한 줄도 안
  들어가는 크로미엄 내장 페이지가 똑같이 깨졌으므로, 원인은 앱이 아니라 런타임
  리소스였다. 앱이 자기 파일을 남이 지우는 자리에 두고 살 수는 없다.
