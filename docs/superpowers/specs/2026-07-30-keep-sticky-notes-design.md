...
| # | 위험 | 막혔을 때의 영향 | 완화책 |
|---|---|---|---|
| R1 | ~~구글이 내장 웹뷰 로그인을 차단 (`disallowed_useragent`)~~ **해소됨 (2026-07-31)** | — | 사용자가 `npm start` 로 실측: 로그인·동의 후 `oauth_token` 쿠키 획득 성공. 차단되지 않음 |
...
R1 결론: 구글은 2023-07-24부터 임베디드 웹뷰의 OAuth 2.0 인가 엔드포인트 요청을 차단한다. `EmbeddedSetup`은 안드로이드 기기 설정용 웹뷰 전용 페이지로 성격이 다르므로 차단 대상인지 불확실했으나, **2026-07-31 실측 결과 차단되지 않았다.** Electron `BrowserWindow` 로 로드해 로그인·동의를 마치면 `oauth_token` 쿠키를 정상적으로 얻는다. 따라서 §8.1 의 내장 로그인이 기본 경로이고, 수동 붙여넣기는 실제 폴백으로만 남는다.
